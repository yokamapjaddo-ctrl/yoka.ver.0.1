 ...
 *   node ingest.js --municipality=fujisawa_kanagawa --dataset=disaster --dry-run
 *
...
import feedTransformer from './transformers/feedTransformer.js';
import facilityTransformer from './transformers/facilityTransformer.js';
...
const TRANSFORMERS = {
  garbage: garbageTransformer,
  feed: feedTransformer,
...
async function main() {
  ...
  try {
    ...
    if (!records.length) {
      throw new Error(`整形後の件数が0件です。transformers/${source.transformer}Transformer.js の列名候補を実データに合わせてください。`);
    }
    ...
    log.summary('取り込み結果', {
      ...
      データ種別: `${datasetKey} → category=${source.category}`,
      取得件数: fetchedCount,
      整形成功: records.length,
      取り込めず: skippedCount,
      新規: stats.inserted,
